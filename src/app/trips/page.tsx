...
function CommandButton({ icon, color, borderColor, onClick, loading, title }: {
  icon: string; color: string; borderColor: string; onClick: () => void; loading: boolean; title: string;
}) {
  return (
    <button
      ...
      disabled={loading}
      title={title}
      className={`w-9 h-9 rounded-lg border-2 flex items-center justify-center text-sm transition-all hover:scale-105 active:scale-95 disabled:opacity-40 ${borderColor} ${color} bg-transparent`}
    >
                    ...
                    {res.carId && (
                      <div className="flex gap-2 sm:flex-col sm:gap-2">
                        <div className="flex gap-2">
                          <CommandButton
                            icon="🔒" color="text-green-600" borderColor="border-green-300 hover:border-green-500"
                            onClick={() => sendCommand('lock', res.carId!)}
                            loading={!!actionLoading[`${res.carId}-lock`]} title="Lock"
                          />
                          <CommandButton
                            icon="🔓" color="text-orange-500" borderColor="border-orange-300 hover:border-orange-500"
                        ...
                        <div className="flex gap-2">
                          <CommandButton
                            icon="⚡" color="text-blue-500" borderColor="border-blue-300 hover:border-blue-500"
                            onClick={() => sendCommand('unkill', res.carId!)}
                            loading={!!actionLoading[`${res.carId}-unkill`]} title="Enable Engine"
                          />
                          <CommandButton
                            icon="🚫" color="text-red-500" borderColor="border-red-300 hover:border-red-500"
